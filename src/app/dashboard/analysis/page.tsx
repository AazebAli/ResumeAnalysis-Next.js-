...
// ─── Section keywords ─────────────────────────────────────────────────────────
// Add any headings from your resume here if they don't split correctly.
const SECTION_KEYWORDS = [
  'EDUCATION', 'EXPERIENCE', 'WORK EXPERIENCE', 'INTERNSHIP', 'INTERNSHIPS',
  'SKILLS', 'SKILLS & INTERESTS', 'TECHNICAL SKILLS',
  'PROJECTS', 'RESEARCH & ACADEMIC PROJECTS', 'ACADEMIC PROJECTS',
  ...
  'INTERESTS', 'SUMMARY', 'OBJECTIVE', 'PUBLICATIONS', 'ACHIEVEMENTS',
];

// ─── Normalise blob text from unpdf ──────────────────────────────────────────
// unpdf strips all newlines, returning one giant string. This injects breaks
// before known section headings and bullet characters so the parser can split.
function normaliseText(raw: string): string {
  let text = raw;

  // 1. Inject double newline before every known section keyword (whole-word, case-insensitive)
  ...
  // 2. Inject newline before bullet/list characters
  text = text.replace(/([•●◦▪▸*])/g, '\n$1');

  // 3. Inject newline before month-year date patterns  e.g. "Jan 2025", "Oct 2023 –"
  text = text.replace(
    /(\b(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\.?\s+\d{4})/g,
    '\n$1'
  );

  // 4. Inject newline before " – " dash separators used between job/project entries
...
function parseResumeText(raw: string): ResumeSection[] {
  ...
  const isHeading = (line: string): boolean => {
    ...
    // Matches known keywords exactly
    if (SECTION_KEYWORDS.some(k => upper === k || upper.startsWith(k + ' '))) return true;
    // All-caps line under 60 chars (typical PDF section header)
    if (/^[A-Z][A-Z\s&/\-]{3,}$/.test(line) && line.length < 60) return true;
  ...
  return sections.length ? sections : [{ heading: null, lines }];
}
        ...
        <div key={si}>
          {sec.heading && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
              <div style={{ width: 3, height: 14, background: C.gold, flexShrink: 0 }} />
              <p style={{
                ...mono, fontSize: 10, fontWeight: 700, color: C.ink, margin: 0,
                letterSpacing: '0.12em', textTransform: 'uppercase',
              }}>
                {sec.heading}
              </p>
          ...
          <div style={{
            ...
            paddingLeft: sec.heading ? 13 : 0,
          }}>
            {sec.lines.map((line, li) => {
              const isBullet = /^[•●◦▪▸*\-–]/.test(line);
              const cleanLine = isBullet ? line.replace(/^[•●◦▪▸*\-–]\s*/, '') : line;
              const isContact = /[@+\d\(\)]{4,}|https?:\/\/|linkedin|github/i.test(line);
              const isNameLine = si === 0 && li === 0 && !sec.heading && line.length < 50;

              return (
                <div key={li} style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                  {isBullet && (
                    <span style={{ color: C.gold, flexShrink: 0, fontSize: 10, marginTop: 4 }}>▸</span>
                  )}
                  <p style={{
                    margin: 0,
                    lineHeight: 1.75,
                    ...(isNameLine
                      ? { ...serif, fontSize: 15, fontWeight: 700, color: C.ink }
                      : { ...mono, fontSize: 11, color: isContact ? C.muted : C.ink }),
                  }}>
                    {cleanLine}
                  </p>
                </div>
